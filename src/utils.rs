...
pub async fn add_nft_approval(nft_program: ActorId, to: ActorId, token_id: TokenId) {
    msg::send_for_reply_as::<_, NFTApproval>(nft_program, NFTAction::Approve { to, token_id }, 0)
        .expect("Error during a sending `NFTAction::Approve` to an NFT program")
        .await
        .expect("Unable to decode `NFTApproval`");
}
